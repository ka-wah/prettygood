...
CONFIG = {
    ...
    "MNY_STD_CAP_LO": -10,   # set e.g. -6.0, or None to disable lower cap
    "MNY_STD_CAP_HI":  10.0,   # set e.g.  4.0, or None to disable upper cap

    # dh_ret caps
    "APPLY_DH_RET_CAP": False,
    "DH_RET_CAP_LO": -5.0,    # e.g. -10.0 if you want to allow more downside
    "DH_RET_CAP_HI":  10.0,    # e.g.  3.0 if you want to clamp upside more
...
def build_next_trading_day_map(dates: pd.Series) -> dict:
    ...
    d = pd.Index(d.date)
    nxt = pd.Series(d[1:], index=d[:-1]).to_dict()
    return nxt
...
# Futures RV & Skew
    ...
    def process_sentiment(
        ...
        date_col: str = "date",
        lag_days: int = 1,           # close -> use t-1 by default
        z_method: str = "rolling",   # "rolling" or "expanding"
        window: int = 60,            # rolling window length (if rolling)
        min_periods: int = 60,       # usually = window; can be smaller
        ddof: int = 0,
    ) -> pd.DataFrame:
        ...
        def _z_past_only(x: pd.Series) -> pd.Series:
            ...
            # lag so features on date t only use info available by close of t-1
        ...
        if val_col is not None:
            ...
            tmp = raw[["date", v]].rename(columns={v: "gt_level"})
    ...
    process_sentiment(
        "google_trends",
        val_col="value",   # whatever the numeric column is called in that csv
        z_method="rolling",
        lag_days=1,        # close-to-close safe default
        window=60,
        min_periods=30
    ),
    on="date",
   ...
   # Next Day Valuation
    ...
    # enforce alignment BEFORE combining
    df.loc[~is_next_td, ["mid_tp1", "dte_tp1"]] = np.nan

    mid_sig = add_nextday_mid_by_signature(df, next_td)  # must also enforce next_td inside
    ...
    if CONFIG.get("APPLY_DH_RET_CAP", True):
        df["dh_ret"] = cap_series(
            df["dh_ret"],
            lo=CONFIG.get("DH_RET_CAP_LO", None),
            hi=CONFIG.get("DH_RET_CAP_HI", None),
        )
        # keep consistency if you want dh_ret == y_price everywhere
    ...
    if CONFIG.get("APPLY_MNY_STD_CAP", True):
        df["moneyness_std"] = cap_series(
            df["moneyness_std"],
            lo=CONFIG.get("MNY_STD_CAP_LO", None),
            hi=CONFIG.get("MNY_STD_CAP_HI", None),
        )
    ...
    # Distribution Diagnostics: moneyness_std (NEW)
    try:
        mny_col = "moneyness_std"
        # if you keep a capped column, prefer it:
        if "moneyness_std_w" in df_out.columns:
            mny_col = "moneyness_std_w"
    ...
    # --- Export extreme y_price observations (NEW) ---
    outdir = CONFIG["outputs_dir"]
    outdir.mkdir(parents=True, exist_ok=True)

    thr = 10.0

    # If you cap at 10 somewhere, >10 may be empty; consider using >= 10 - eps
    eps = 1e-12
    mask_gt = to_num(df["y_price"]) > thr
    mask_eqclip = to_num(df["y_price"]) >= (thr - eps)

    # Choose which you want:
    mask = mask_gt  # strict > 10
    # mask = mask_eqclip  # use this if y_price is clipped at 10

    cols_out = [
        # identifiers
        "date","optionid", "optionsymbol", "futuresymbol",
        # contract
        "expiration", "strike", "callput", "is_call", "dte", "tau",
        # state/prices
        "F_t", "F_tp1", "mid_t", "V_tp1", "V_tp1_modeled_flag",
        # core risk/greeks
        "iv", "delta_model", "gamma_model", "vega_model", "theta_model",
        # moneyness
        "log_moneyness", "moneyness_std",
        # microstructure
        "opt_rel_spread_raw", "opt_rel_spread_final", "opt_rel_spread_imputed_flag",
        # target decomposition
        "dh_pnl", "y_price", "dh_ret",
    ]

    cols_out = [c for c in cols_out if c in df_out.columns]

    y10 = df_out.loc[mask, cols_out].copy()
    y10 = y10.sort_values("y_price", ascending=False)

    # Save both CSV and parquet (CSV easy to inspect; parquet preserves dtypes)
    y10.to_csv(outdir / "y_price_gt_10_rows.csv", index=False)
    y10.to_parquet(outdir / "y_price_gt_10_rows.parquet", index=False)

    print(f"[DIAG] Exported y_price outliers: {len(y10)} rows -> {outdir}/y_price_gt_10_rows.*")

    # Save main panel
    df_out.to_parquet(outdir / CONFIG["panel_out"], index=False)

    # -------------------- NEW: automatic DTE splits --------------------

    def _save_dte_split(df: pd.DataFrame, lo: int, hi: int, fname: str) -> None:
        sub = df[df["dte"].between(lo, hi)].copy()
