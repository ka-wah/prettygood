...
def r2_os_winsor_aligned(y_true: np.ndarray, y_pred: np.ndarray, lo: float, hi: float) -> float:
    ...
    return 1.0 - msfe_model / msfe_naive
# =============================================================================
# Weighted Metrics (Updated with CW_t_w and CW_t_w_XS)
# =============================================================================
...
def fit_family(
    ...
    dates_tr=None,
    dates_va=None,
    sample_weights_tr=None,  # NEW
    sample_weights_va=None,  # NEW
    ...
    """Model selection per family using time-grouped CV with optional sample weighting."""
    import inspect
    try:
    ...
    def _fit_with_optional_weight(est, X, y, sample_weight):
        if sample_weight is None:
            est.fit(X, y)
            return
        # Normalize weights to avoid optimizer scale issues
        ...
        if isinstance(est, SkPipeline):
            ...
            try:
                ...
                if "sample_weight" in sig_last.parameters:
                    params[f"{last_name}__sample_weight"] = w
                    est.fit(X, y, **params)
                    return
            except Exception:
                pass
            # Try setting on steps (e.g. for XGB/LGBM in pipeline)
            for nm, step in est.steps:
                try:
                    if "sample_weight" in inspect.signature(step.fit).parameters:
                        params[f"{nm}__sample_weight"] = w
                except Exception:
                    continue
            if params:
                est.fit(X, y, **params)
                return
        # Fallback: ignore weights if model doesn't support them
        est.fit(X, y)
    ...
    if use_trva and (dates_tr is not None and dates_va is not None):
        ...
        # Stack weights if provided
        if sample_weights_tr is not None and sample_weights_va is not None:
            w_cv = np.hstack([np.asarray(sample_weights_tr), np.asarray(sample_weights_va)])
        elif sample_weights_tr is not None: 
            # Fallback if va weights missing (shouldn't happen in logic)
            w_cv = np.hstack([np.asarray(sample_weights_tr), np.ones(len(z_va))])
        else:
            w_cv = None
    else:
        X_cv = X_tr
        ...
        w_cv = np.asarray(sample_weights_tr) if sample_weights_tr is not None else None
    ...
    for name in family_names:
        ...
        for params in _iter_param_sets(spec.grid_default, strategy=strategy, n_iter=n_iter, seed=seed):
            ...
            for tr_idx, va_idx in splits:
                ...
                # Combine Date Weights and Sample Weights
                w_tr_fold = _equal_day_weights(d_cv[tr_idx]) if d_cv is not None else np.ones(len(tr_idx))
                if w_cv is not None:
                    w_tr_fold *= w_cv[tr_idx]
                
                # Validation weights (for early stopping)
                w_va_fold = _equal_day_weights(d_cv[va_idx]) if d_cv is not None else np.ones(len(va_idx))
                if w_cv is not None:
                    w_va_fold *= w_cv[va_idx]
                ...
                if is_lgbm:
                    ...
                    if use_early_stop:
                        ...
                        # Pass weights to eval set if possible
                        fit_kwargs["eval_sample_weight"] = [np.asarray(w_va_fold)]
                        try:
                    ...
                    _fit_with_optional_weight(est, Xtr, ztr, w_tr_fold)
                    zhat_va = est.predict(Xva)
        ...
        for j, (best_score, best_params, best_n_rounds) in enumerate(top, start=1):
            ...
            if use_trva and (dates_tr is not None and dates_va is not None):
                ...
                # Combine weights for Refit
                if sample_weights_tr is not None and sample_weights_va is not None:
                    w_fit_sample = np.hstack([sample_weights_tr, sample_weights_va])
                elif sample_weights_tr is not None:
                    w_fit_sample = np.hstack([sample_weights_tr, np.ones(len(z_va))])
                else:
                    w_fit_sample = None
            else:
                X_fit = X_tr
                ...
                w_fit_sample = sample_weights_tr

            # Final date weights * sample weights
            w_fit_combined = _equal_day_weights(d_fit) if d_fit is not None else np.ones(len(z_fit))
            if w_fit_sample is not None:
                w_fit_combined *= w_fit_sample
            ...
            _fit_with_optional_weight(est, X_fit, z_fit, w_fit_combined)
...
def export_metrics(
    ...
    *,
    ...
    pd.DataFrame(rows).sort_values("R2_OS", ascending=False).round(6).to_csv(os.path.join(out_dir, filename), index=False)
...
def export_metrics_by_contract_bins(
    ...
    *,
    ...
    df_out.round(6).to_csv(os.path.join(out_dir, "metrics_by_contract.csv"), index=False)
    return df_out
...
def export_portfolios_raw(
    ...
    *,
    ...
    if not daily_frames:
        pd.DataFrame(columns=["group", "model", "ls_return"]).round(6).to_csv(os.path.join(out_dir, "portfolios_daily.csv"), index=False)
        pd.DataFrame(columns=["model", "N", "mean", "vol", "Sharpe_ann", "Hit_ratio"]).round(6).to_csv(
    ...
    pd.DataFrame(rows).sort_values("model").round(6).to_csv(os.path.join(out_dir, "portfolio_metrics.csv"), index=False)
...
def export_shap_n_en(
    ...
    k_bg = int(sconf.get("background_kmeans", 50))
    check_add = bool(sconf.get("check_additivity", False))
    max_display = int(sconf.get("max_display", 10))
